...
const ReadingLog = () => {
  ...
  const isSubmitDisabled = !readToday || summary.length < 50;

  const handleReadTodayChange = (e) => {
    setReadToday(e.target.checked);
  };
      ...
      <div className="mx-auto max-w-2xl mt-8 p-4 bg-white rounded-lg shadow">
        <label className="block mb-4">
          <input
            type="checkbox"
            checked={readToday}
            onChange={handleReadTodayChange}
            className="mr-2"
          />
          <span className="text-lg font-semibold">Did you read for at least 15 minutes today?</span>
        </label>

        {readToday && (
          <div className="mb-4">
            <label className="block text-lg font-semibold mb-2">Write a 50-word summary:</label>
            <textarea
              value={summary}
